...
int main(int argc, char* argv[])
{
    ...
    double cv_ts = 1000.0 * (t3 - t2) / cv::getTickFrequency();

    // Print feature extraction details
    std::cout << "CUDA AKAZE KeyPoints Image1: " << kpts1.size() << " Image2: " << kpts2.size() << std::endl;
    ...
    // Store match results
    std::cout << "CUDA/OpenCV Diff Ratio: " << compareMatches(gpu_dmatches, cv_dmatches) << std::endl;
                      ...
                      const std::vector<std::vector<cv::DMatch>>& cv_matches)
{
    ...
    return static_cast<double>(differing_matches) / static_cast<double>(total_matches * 2);
}
