...
std::string to_formatted_string(Integer x, int num_digits)
{
    std::stringstream oss;
...
/// Convert the scale of the input image prior to visualisation
void convert_scale(cv::Mat& src);

/// This function copies the input image and converts the scale of the copied image prior visualisation
void copy_and_convert_scale(const cv::Mat& src, cv::Mat& dst);

/// This function draws the list of detected keypoints
void draw_keypoints(cv::Mat& img, const std::vector<cv::KeyPoint>& kpts, bool draw_point_size = true);
...
/// This function saves the matches to a regular ASCII file
/// @param outFile Name of the output file where the matches will be stored
/// @param matches Vector of matches
int save_matches(const std::string& outFile, const std::vector<std::vector<cv::DMatch>>& matches);

/// This function converts matches to points using the nearest neighbour distance
...
/// @param matches Vector of nearest neighbours for each keypoint
...
/// This function computes the set of inliers estimating the fundamental matrix or a planar homography
/// in a RANSAC procedure
/// @param matches Vector of putative matches
/// @param inliers Vector of inliers
/// @param error The minimum pixel error to accept an inlier
/// @param use_fund Set to true if you want to compute a fundamental matrix
void compute_inliers_ransac(const std::vector<cv::Point2f>& matches, std::vector<cv::Point2f>& inliers, float error,
                            bool use_fund);
...
/// @param min_error The minimum pixel error to accept an inlier
...
/// This function draws the set of the matches between the two images (up and down)
/// @param img1 First image
/// @param img2 Second image
/// @param img_com Image with the matches
/// @param ptpairs Vector of point pairs with the set of matches
/// @param color The colour for each method
...
/// This function draws the set of the matches between the two images (size by size)
/// @param img1 First image
/// @param img2 Second image
/// @param img_com Image with the matches
/// @param ptpairs Vector of point pairs with the set of matches
